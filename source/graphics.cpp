...
namespace SGL
{
    ...
    enum BuiltinMeshType
    {
        BUILTIN_MESH_CUBE = 0,
        BUILTIN_MESH_SPHERE,
        BUILTIN_MESH_CAPSULE,
        BUILTIN_MESH_CYLINDER,

        BUILTIN_MESH_COUNT
    };

    static int BuiltinMeshHandles[BUILTIN_MESH_COUNT] = { -1, -1, -1, -1 };
    ...
    void GraphicsInit(int width, int height, const char* title)
    {
        ...
        BuiltinMeshHandles[BUILTIN_MESH_CUBE] = RegisterMesh(GenMeshCube(1.0f, 1.0f, 1.0f));
        BuiltinMeshHandles[BUILTIN_MESH_SPHERE] = RegisterMesh(GenMeshSphere(0.5f, 16, 16));
        BuiltinMeshHandles[BUILTIN_MESH_CAPSULE] = RegisterMesh(GenMeshCapsule(0.5f, 1.0f, 16, 16));
        BuiltinMeshHandles[BUILTIN_MESH_CYLINDER] = RegisterMesh(GenMeshCylinder(0.5f, 1.0f, 16));

        for (int i = 0; i < BUILTIN_MESH_COUNT; i++)
        {
            int handle = BuiltinMeshHandles[i];

            if (!IsMeshValid(handle)) continue;

            GetMeshModel(handle).materials[0].shader = StandardShader;
        }
    }
    ...
    {
        return BuiltinMeshHandles[BUILTIN_MESH_CUBE];
    }

    int GetBuiltinSphereMeshHandle()
    {
        return BuiltinMeshHandles[BUILTIN_MESH_SPHERE];
    }

    int GetBuiltinCapsuleMeshHandle()
    {
        return BuiltinMeshHandles[BUILTIN_MESH_CAPSULE];
    }

    int GetBuiltinCylinderMeshHandle()
    {
        return BuiltinMeshHandles[BUILTIN_MESH_CYLINDER];
    }
    ...
    void GraphicsRender()
    {
        ...
        for (int i = 0; i < (int)list.size(); i++)
        {
            ...
            if (!IsMeshValid(item.meshID)) continue;
            ...
            Model& model = GetMeshModel(item.meshID);
            model.transform = MakeTransformMatrix(item.transform);
